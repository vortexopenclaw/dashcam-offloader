    ...
    static func run() -> Bool {
        do {
            ...
            try FileManager.default.createDirectory(
                at: blackvueElite8Source.appendingPathComponent("BlackVue/Config", isDirectory: true),
            ...
            try FileManager.default.createDirectory(
                at: blackvueDR970XLteSource.appendingPathComponent("BlackVue/Config", isDirectory: true),
                withIntermediateDirectories: true
            )
            try FileManager.default.createDirectory(
                at: blackvueDR970XLteSource.appendingPathComponent("BlackVue/Record", isDirectory: true),
                withIntermediateDirectories: true
            )
            try FileManager.default.createDirectory(
                at: blackvueDR970XLteFirmware2Source.appendingPathComponent("BlackVue/Config", isDirectory: true),
                withIntermediateDirectories: true
            )
            try FileManager.default.createDirectory(
                at: blackvueDR970XLteFirmware2Source.appendingPathComponent("BlackVue/Record", isDirectory: true),
                withIntermediateDirectories: true
            )
            try FileManager.default.createDirectory(
                at: blackvueDR970XSource.appendingPathComponent("BlackVue/Config", isDirectory: true),
                withIntermediateDirectories: true
            )
            try FileManager.default.createDirectory(
            ...
            try Data("version = 2.00\nmodel = DR970X LTE Plus\n".utf8)
                .write(to: blackvueDR970XLteSource.appendingPathComponent("BlackVue/Config/micom_version.bin"))
            try Data("[firmware]\nversion = 2.007\nmodel = DR970X LTE Plus\n[config]\nversion = 1.071\n[revision]\nrev = 1584\n".utf8)
                .write(to: blackvueDR970XLteFirmware2Source.appendingPathComponent("BlackVue/Config/version.bin"))
            try Data("[micom]\nversion = 2.00\nmodel = DR970X LTE Plus v2.007(rev1584)\n".utf8)
                .write(to: blackvueDR970XLteFirmware2Source.appendingPathComponent("BlackVue/Config/micom_version.bin"))
            try Data("version = 2.008\nmodel = DR970X Plus\n".utf8)
                .write(to: blackvueDR970XSource.appendingPathComponent("BlackVue/Config/version.bin"))
            try Data("version = 2.01\nmodel = DR970X Plus v2.008(rev1568)\n".utf8)
                .write(to: blackvueDR970XSource.appendingPathComponent("BlackVue/Config/micom_version.bin"))
            try Data(repeating: 3, count: 1024)
            ...
            try Data(repeating: 8, count: 1024)
                .write(to: blackvueDR970XLteSource.appendingPathComponent("BlackVue/Record/20260101_120000_NR.mp4"))
            try Data(repeating: 9, count: 1024)
                .write(to: blackvueDR970XLteFirmware2Source.appendingPathComponent("BlackVue/Record/20260101_120000_NF.mp4"))
            try Data(repeating: 10, count: 1024)
                .write(to: blackvueDR970XLteFirmware2Source.appendingPathComponent("BlackVue/Record/20260101_120000_NR.mp4"))
            try Data(repeating: 5, count: 1024)
                .write(to: blackvueDR970XSource.appendingPathComponent("BlackVue/Record/20260101_120000_NF.mp4"))
            try Data(repeating: 6, count: 1024)
                .write(to: blackvueDR970XSource.appendingPathComponent("BlackVue/Record/20260101_120000_NR.mp4"))
            ...
            guard dr970xLteScan.candidates.first?.profile.id == "blackvue-dr970x-lte-plus" else {
                print("SMOKE FAIL: DR970X LTE Plus was not top BlackVue candidate")
                return false
            }
            let dr970xLteFirmware2Scan = try scanner.scan(sourceURL: blackvueDR970XLteFirmware2Source, profiles: profiles)
            guard dr970xLteFirmware2Scan.candidates.first?.profile.id == "blackvue-dr970x-lte-plus" else {
                print("SMOKE FAIL: DR970X LTE Plus firmware 2 was not top BlackVue candidate")
                return false
            }
            let dr970xScan = try scanner.scan(sourceURL: blackvueDR970XSource, profiles: profiles)
            guard dr970xScan.candidates.first?.profile.id == "blackvue-dr970x-plus" else {
                print("SMOKE FAIL: DR970X Plus was not top BlackVue candidate")
                return false
            }
