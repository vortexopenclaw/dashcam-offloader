...
These files are the OcuSync live-view transmission stream cached locally by DJI Fly during flight. They are not high-resolution recordings. The live-view feed is a compressed preview stream used for pilot visibility. Codec and resolution not confirmed from ffprobe because these files are not primary footage.
