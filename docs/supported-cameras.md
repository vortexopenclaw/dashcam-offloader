...
- 70mai X800 - profile drafted from real app learning submission `91e46abf-0eea-40fb-9335-0931bfd45be7` and live mounted card `/Volumes/70MAI_X800`. 2-channel F/R card with full-resolution folders `Normal/Front`, `Normal/Rear`, `Parking/Front`, `Parking/Rear`, `Lapse/Front`, `Lapse/Rear`, observed front proxy folders `.s_Front`, and filename pattern `NO`/`PA`/`LA`/`EV` + `YYYYMMDD-HHMMSS-SEQUENCE` + channel suffix.
